# ablation/reasoning_abl/selector_abl.py
...
SelectionMode = Literal["score", "first", "random"]
...
def select_for_next(
    ...
    tau: float = 0.08,
    selection_mode: SelectionMode = "score",
    ...
    selection_mode:
      - score  : normal ToT selection using ranking + tau/gap
      - first  : keep only the first generated/ranked candidate
      - random : keep one random candidate
    """
    ...
    if selection_mode == "score":
        ...
        kept = ranked[:1] if ranked else []

    elif selection_mode == "random":
        rr = rng if rng is not None else random.Random(0)
        kept = [rr.choice(ranked)] if ranked else []
