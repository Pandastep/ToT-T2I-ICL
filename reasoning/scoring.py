...
def score_query_anchor(thought: str, hypothesis: Hypothesis) -> float:
    """
    Reward thoughts that remain anchored to the target query or
    explicitly refer to the target subject.
    """
    text = _norm(thought)
    ...
    # soft fallback if query is empty
    if not q_words:
    ...
    # If replacement pattern is present but query anchor is weak, penalize hard
...
def score_entity_preservation(thought: str, hypothesis: Hypothesis, config: ScoreConfig) -> float:
    """
    Penalize unsupported substitution of the central entity.
    This is benchmark-agnostic: it does not assume which attribute changes.
    """
    text = _norm(thought)
    if not text:
        return 0.0
    ...
    # strong positive: preserve target or refer to it explicitly
    if overlap >= 0.5 or "target object" in text or "subject" in text:
        score += 0.2

    # penalize hard substitutions
    if replacement_hits > 0 and overlap < 0.35:
        score -= 0.35

    # penalize introducing unrelated concrete entities
    if extra_entity_hits > 0 and overlap < 0.35:
        score -= min(0.25, 0.08 * extra_entity_hits)

    # penalize abrupt semantic jumps with little relation to query
...
def score_rule_consistency(thought: str, hypothesis: Hypothesis) -> float:
    """
    Keep this weak and generic.
    It measures broad alignment with rule/summary, but it should not dominate.
    """
    text = _norm(thought)
    if not text:
        return 0.0
...
def score_constraints_consistency(thought: str, hypothesis: Hypothesis) -> float:
    """
    Generic natural-language constraint consistency.
    No artificial floor like max(0.4, overlap).
    """
    text = _norm(thought)
    ...
    for c in constraints:
        ...
        if any(neg in c_norm for neg in ["never", "must not", "do not", "should not"]):
            ...
            # neutral baseline, but much weaker than before
            if overlap >= 0.4:
                scores.append(0.85)
            elif overlap >= 0.2:
                scores.append(0.65)
...
def score_stage_fidelity(thought: str, stage: str, config: ScoreConfig) -> float:
    """
    Score whether the thought plays the correct role for the stage.
    This is still heuristic, but stage-role-based rather than benchmark-based.
    """
    text = _norm(thought)
    if not text:
        return 0.0
...
def score_quality(thought: str, config: ScoreConfig, stage: str = "") -> float:
    """
    Auxiliary score only.
    It should reward concise, readable, interpretable thoughts,
    but not dominate semantic quality.
    """
    text = _norm(thought)
    if not text:
        return 0.0
...
def score_unsupported_leap_penalty(thought: str, hypothesis: Hypothesis, config: ScoreConfig) -> float:
    """
    Penalize abrupt transformations with weak connection to query.
    """
    text = _norm(thought)
    if not text:
        return 0.0
...
def score_redundancy_penalty(thought: str, previous_thoughts: Optional[List[str]], config: ScoreConfig) -> float:
    """
    Penalize near-duplicates of earlier thoughts in the current branch.
    """
